...
const ContentFlowModule = () => {

    var CFElement;

    var window = d_win.global;
    ...
    var ContentFlowGlobal = {
        ...
        Browser: new (function () {
            ...
            this.IE9 = this.IE && isIE === 9;
            this.IE7 = this.IE && !this.IE6 && !this.IE8 && !this.IE9 ? true : false;
        ...
        initPath: function () {
            ...
            this.BaseDir = require.toUrl("../lib/");
        ...
        onloadInit: function onloadInit(...args) {
            // quit if this function has already been called
            args.callee = onloadInit;
            if (args.callee.done) return;
...
//TODO: This was changed by ome!
    ...
    ContentFlowAddOn.prototype = {
        ...
        addStylesheet: function (path) {
            // changed by mre: disable stylesheets (loaded by themes)
            /*
             if (!path)
    ...
    var ContentFlowGUIElement = function (CFobj, element) {
        ...
        element.makeDraggable = function (onDrag, beforeDrag, afterDrag) {
            ...
            this.initDrag = function (event) {
                ...
                beforeDrag(event);
                //PATCHED after init we start dragging
            ...
            //PATCHED added the before drag callback to the workflow
            if (this.Browser.iPhone) {
                this.addEventListener('touchstart', this.initDrag, false);
            }
            else {
                this.addEvent('mousedown', this.initDrag, false);
            }
    ...
    var ContentFlowItem = function (CFobj, element, index) {
        ...
        this.clickItem = function (event) {
            ...
            //console.debug("cover index : "+index);
        ...
        if (this.content.nodeName == "IMG") {
            if (this.content.getAttribute("width")) {
                ...
                } else if (this.Browser.IE && !this.Browser.IE9 && !this.content.onload) {
                    var self = this;
                    var t = window.setInterval(function () {
                        // I'm not sure but this break in IE:
    ...
    ContentFlowItem.prototype = {
        ...
        initClick: function () {
            ...
            this[this._activeElement].addEvent('dblclick', cItem, false);
        },
        ...
        setImageFormat: function (img) {
            ...
            if (img.getAttribute("width")) {
                ...
                //FIX for IE8 and other browsers
                var image = new Image();
                image.src = img.src;
                width = image.width;
                height = image.height;
            ...
            var origProportion = width / height;
            img.width = width;
            img.height = height;
            img.origProportion = origProportion;
            img.setAttribute('origProportion', origProportion);
            img.addClassName(origProportion <= 1 ? 'portray' : 'landscape');
            //console.debug("ContentFlow.setImageFormat: origProportion="+origProportion+"; w: "+width+"; h: "+height);
        },
    ...
    ContentFlow.prototype = {
        ...
        _defaultConf: {
            ...
            //PATCHED by fba to get a callback once we are done dragging
        ...
        init: function () {
            ...
            // PATCH END //
            var delta = ContentFlowGlobal.earliestInitTimestamp - new Date().getTime();
            if (delta > 0) {
                var that = this;
                setTimeout(function () {
                    that._init();
        ...
        _addItem: function (el, index) {
            ...
            var CF = this;
            clearTimeout(this._stepUpdateTimer);
            this._stepUpdateTimer = window.setTimeout(function () {
                if (CF.items.length == 1 || !CF._activeItem) {
        ...
        _checkVisibleItems: function (reset) {
            var length = this.items.length;
            var odd = length % 2 === 0 ? length - 1 : length;
            var visibleItems = this.conf.visibleItems;
            if (visibleItems <= 0 || reset) {
                var org = this.conf.origVisibleItems;
                if (org < 0) {
                    org = odd;
                }
                visibleItems = Math.min(org, odd);
            }
            this.conf.visibleItems = Math.max(visibleItems, 1);
        },
        ...
        _initGUI: function () {
            ...
            if (this.conf.flowDragFriction > 0) {
                //PATCHED by fba introduced counter to just calculate the drag stuff on the fifth call
                var dragCounter = 0;
                var onDrag = function (event) {
                    if (dragCounter === DRAG_INTERVALL) {
                        var e = event;
                        if (event.touches) e = event.touches[0];
                ...
                var beforeDrag = function (event) {
                    //PATCHED by fba to get a callback on drag start
                    this.conf.onDragStart();
                }.bind(this);
                ...
                var afterDrag = function (event) {
                    ...
                    //PATCHED by fba to get a callback on drag end
                    this.conf.onDragEnd(this._activeItem);
                }.bind(this);
            ...
            if (this.Slider) {
                ...
                //PATCHED by fba introduced counter to only execute drag code on fifth time, otherwise its not working in ie8
                var dragCounter = 0;
                var that = this;
                var onDrag = function (event) {
                    if (dragCounter === DRAG_INTERVALL) {
                        that._sliderDragHandler(event);
                        dragCounter = 0;
                    } else {
                        dragCounter++;
                    }
            ...
            if (this.Position) {
                this.Position.setLabel = function (index) {
                    ...
                    if (this.items && this.items[index] && this.items[index].label)
        ...
        _sliderDragHandler: function (event) {
            var e = event;
            if (e && e.touches) e = e.touches[0];
            var selectedIndex = 1;
            if (e) {
                selectedIndex = this._checkIndex((e.clientX - this.Scrollbar.position.left) / this.Scrollbar.dimensions.width * this.itemsLastIndex);
            }
            this._currentPosition = this._currentPosition % this.items.length;
            //                this._targetPosition = this._getPositionByIndex(selectedIndex);
            //PATCHED we patched the targetposition so it won´t wiggle between item indexes
            ...
            //PATCHED by fba to ensure onMoveTo called when slider is dragged to shwo the according item label
            this.conf.onMoveTo(this._getItemByPosition(this._targetPosition));
        },
                ...
                if (SL = this.Slider) {
                    ...
                    if (PO = this.Position) {
                        ...
                        if (extraSpace > 0) {
                            ...
                            extraSpace *= -1;
                            // Fix for Bug MAPAPPS-998
                            // TODO: Check if this causes problems if Map Flow is used in other ways than in default
        ...
        _step: function () {
            ...
            //console.debug("contentflow_src: _step:"+absDiff + " targetPosition:"+this._targetPosition  +" currentPosition:"+this._currentPosition + " visibleItems:"+this.conf.visibleItems);
            if (this.items.length && absDiff > 0.001) { // till activeItem is nearly at position 0
        ...
        _setItemSize: (function () {
            if (ContentFlowGlobal.Browser.IE) {
                var _setItemSize = function (item, size) {
                    ...
                    if (item.reflection) {
                        var h = parseInt(item.content.style.height, 10);
    ...
    //TODO: This was changed by ome! (init() needs to be called manually)
    ContentFlowGlobal.init();

    return {ContentFlowGlobal, ContentFlow};

};

export default ContentFlowModule;